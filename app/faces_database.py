...
class FacesDatabase:
    ...
    def __init__(self, path, face_identifier, landmarks_detector, face_detector=None, no_show=False, augment=True):
        ...
        for path in paths:
            ...
            variants = [image]
            if augment:
                # Flip + brightness + small rotations. With one enrollment
                # photo per person, this gives 6 embeddings per identity,
                # which markedly improves 1-shot matching robustness.
                variants.append(cv2.flip(image, 1))
                variants.append(cv2.convertScaleAbs(image, alpha=1.2, beta=10))
                variants.append(cv2.convertScaleAbs(image, alpha=0.8, beta=-10))
                h_img, w_img = image.shape[:2]
                for deg in (8, -8):
                    M = cv2.getRotationMatrix2D((w_img / 2, h_img / 2), deg, 1.0)
                    variants.append(cv2.warpAffine(image, M, (w_img, h_img),
                                                   borderMode=cv2.BORDER_REPLICATE))

            for variant_idx, variant in enumerate(variants):
                if face_detector:
                    rois = face_detector.infer((variant,))
                    if len(rois) < 1:
                        if variant_idx == 0:
                            log.warning("Not found faces on the image '{}'".format(path))
                        continue
                else:
                    w, h = variant.shape[1], variant.shape[0]
                    rois = [FaceDetector.Result([0, 0, 0, 0, 0, w, h])]

                for idx, roi in enumerate(rois):
                    r = [roi]
                    landmarks = landmarks_detector.infer((variant, r))

                    face_identifier.start_async(variant, r, landmarks)
